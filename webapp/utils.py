...
def format_community_event_time(event):
    ...
    if start_dt.date() == end_dt.date():
        date_part = start_dt.strftime("%d %b,").replace(start_dt.strftime("%b"), start_dt.strftime("%b").capitalize())
        ...
        if start_period == end_period:
            event["formatted_time"] = f"{date_part} {start_time_part}-{end_time_part} {end_period}"
        else:
            event["formatted_time"] = f"{date_part} {start_time_part} {start_period}-{end_time_part} {end_period}"
    else:
        start_time = start_dt.strftime("%d %b, %I:%M%p")
        end_time = end_dt.strftime("%d %b, %I:%M%p")
        start_time = start_time.replace(start_dt.strftime("%b"), start_dt.strftime("%b").capitalize())
        end_time = end_time.replace(end_dt.strftime("%b"), end_dt.strftime("%b").capitalize())
        event["formatted_time"] = f"{start_time} - {end_time}"
