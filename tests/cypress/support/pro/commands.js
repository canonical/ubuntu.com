...
Cypress.Commands.add("fillCountryVAT", (country = "AU", vat = "", state = "") => {
    cy.wait(2000)
    cy.findByLabelText("Country/Region:").then($select => { $select.val(country) }).parent().trigger('change')
    if (state) {
        cy.findByLabelText("State:").select(state);
    }
    cy.wait(1000)
    if (vat) {
        cy.findByLabelText("VAT number:").type(vat);
    }
    cy.findByRole("button", { name: "Save" }).click()
});

Cypress.Commands.add("fillInEmail", (email = Cypress.env("UBUNTU_USERNAME")) => {
...
Cypress.Commands.add("selectProducts", (quantity = 1, machineType = "physical", coverage = "uaia", supportType = "", supportInterval = "Weekday") => {
