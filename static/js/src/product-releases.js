...
const subscriptionSelection = document.getElementById('js-subscription-selector');
...
productSelection.addEventListener('change', () => {
	...
	deployments.forEach(deployment => { 
		...
		releaseSelection.options[releaseSelection.options.length] = new Option(releaseName, releaseName);
	}); 
});
...
releaseSelection.addEventListener('change', () => {
	...
	versions.forEach(version => { 
		versionSelection.options[versionSelection.options.length] = new Option(version.release, version.release); 
	});
});
